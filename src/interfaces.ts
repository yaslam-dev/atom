 ...
 */
export interface StoreAdapter<T = any> {
  // Document operations
 ...
 */
export interface PullResult<T = any> {
  success: boolean;
 ...
 */
export interface SyncEvents<T = any> {
  // Document events
  ...
  // Connection events
  [SYNC_EVENT.CONNECTION_ONLINE]: {};
  [SYNC_EVENT.CONNECTION_OFFLINE]: {};
...
export interface EventEmitter<T = any> {
  on<K extends keyof SyncEvents<T>>(event: K, listener: (data: SyncEvents<T>[K]) => void): () => void;
