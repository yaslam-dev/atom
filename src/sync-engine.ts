...
import { now, createDocument, cloneDocument, debounce, retry } from './utils';

export interface SyncEngineConfig<T = any> {
 ...
 */
export class SyncEngine<T = any> implements EventEmitter<T> {
  private store: StoreAdapter<T>;
  private transport: TransportAdapter<T>;
  private conflictResolver: ConflictResolver<T>;
  private changeTracker: ChangeTracker<T>;
  private eventEmitter: SyncEventEmitter<T>;
  ...
  async pull(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.emit(SYNC_EVENT.SYNC_COMPLETED, { type: SYNC_OPERATION.PULL, changeCount: result.changes.length });
      } else {
        throw new Error(result.error || 'Pull failed');
      }
    } catch (error) {
      this.emit(SYNC_EVENT.SYNC_FAILED, { type: SYNC_OPERATION.PULL, error: (error as Error).message });
      throw error;
    } finally {
  ...
  async push(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        ...
        this.emit(SYNC_EVENT.SYNC_COMPLETED, { type: SYNC_OPERATION.PUSH, changeCount: changes.length });
      } else {
        throw new Error(result.error || 'Push failed');
      }
    } catch (error) {
      this.emit(SYNC_EVENT.SYNC_FAILED, { type: SYNC_OPERATION.PUSH, error: (error as Error).message });
      throw error;
    } finally {
      this.isSyncing = false;
      this.emitStateChange();
  ...
  private async initializeState(): Promise<void> {
    try {
      ...
      console.warn('Failed to initialize sync state:', error);
  ...
  private async applyRemoteChanges(changes: ChangeRecord<T>[]): Promise<void> {
    for (const change of changes) {
      try {
        ...
        console.error(`Failed to apply remote change for ${change.id}:`, error);
      }
    }
  }
  ...
  private async handleRemoteChanges(changes: ChangeRecord<T>[]): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to handle remote changes:', error);
    }
  }
